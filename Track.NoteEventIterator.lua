...
-- @class table
-- @name Track.NoteEventIterator
Track.NoteEventIterator = {};

---
-- 初期化を行う
-- @param list (EventList) 反復子の元になるリスト
-- @return (Track.NoteEventIterator)
function Track.NoteEventIterator.new( list )
    local this = {};
    this._list = list;
    this._pos = 0;
    ...
    function this:hasNext()
        ...
        for i = self._pos + 1, count, 1 do
            if( self._list:get( i - 1 ).id.type == IdTypeEnum.Anote )then
                return true;
            end
        end
        return false;
