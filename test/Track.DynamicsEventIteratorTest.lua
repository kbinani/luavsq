...
function test()
    ...
    -- dynamics イベントがひとつも無いので、最初から hasNext が false を返す
    local iterator = luavsq.Track.DynamicsEventIterator.new( list );
    assert_false( iterator:hasNext() );

    -- dynamics イベントをひとつ足してテスト
    local b = luavsq.Event.new( 480, luavsq.Id.new( 0 ) );
    b.id.type = luavsq.IdTypeEnum.Aicon;
    local idB = list:add( b, 2 );
    iterator = luavsq.Track.DynamicsEventIterator.new( list );
    assert_true( iterator:hasNext() );
    local event = iterator:next();
    assert_equal( 480, event.clock );
    assert_equal( 2, event.internalId );
    assert_equal( luavsq.IdTypeEnum.Aicon, event.id.type );
    assert_false( iterator:hasNext() );

    -- remove 後の残っているイベントをチェック
    local c = luavsq.Event.new( 2000, luavsq.Id.new( 0 ) );
    c.id.type = luavsq.IdTypeEnum.Aicon;
    list:add( c, 3 );
    iterator = luavsq.Track.DynamicsEventIterator.new( list );
    ...
    assert_equal( luavsq.IdTypeEnum.Anote, list:getElement( 0 ).id.type );
end
